...
export const binarySearch: Pattern[] = [
  {
    ...
    mentalModel: {
      lines: [
        "Keep an invariant: the answer, if it exists, lies inside [lo, hi).",
        "Compute mid without overflow: lo + (hi-lo)/2.",
        "Every iteration must strictly shrink the range, or you loop forever.",
      ],
  ...
  a[mid] > target → hi = mid
      ...
      key: "Pick one convention — half-open [lo, hi) — and write every variant with it.",
    },
    templates: [
      {
        name: "Classic",
        filename: "binary_search.go",
        note: "Half-open bounds. The loop ends when lo == hi and the range is empty.",
    ...
    return -1
      ...
      },
      {
        ...
        filename: "stdlib.go",
        note: "sort.Search is a lower bound over any predicate that is false-then-true.",
...
i, found := slices.BinarySearch(nums, target)`,
      },
      {
        ...
        note: "A row-major sorted matrix is one sorted array with divmod indexing.",
    ...
    complexity: [
      ...
      { label: "Space", value: "O(1)", note: "iterative; O(log n) if written recursively" },
    ],
    why: "Each comparison rules out half the remaining candidates, so the range shrinks from n to 1 in log₂n steps. The only requirement is that the comparison is consistent with the ordering — which is why 'sorted' is a precondition, not a hint.",
    variations: [
      ...
      { name: "Binary search on answer", detail: "When the array itself is not sorted, but a predicate over the answer space is monotone." },
  ...
  },
  {
    ...
    mentalModel: {
      lines: [
        "Think of the predicate a[i] >= target. It is false, then true.",
        "Lower bound is the boundary index where it flips.",
        "It always returns a value in [0, n] — n means 'everything is smaller'.",
      ],
  ...
  count of elements < 2 is also 1`,
      key: "Lower bound never fails — it returns an insertion point, and you check the element yourself.",
    },
    templates: [
      {
        name: "Lower bound",
        filename: "lower_bound.go",
        note: "First index i with nums[i] >= target. Returns len(nums) if none.",
    ...
    return lo
      ...
      },
      {
        ...
        note: "Two bounds, no special cases, no separate 'find any then walk' step.",
    ...
    return []int{lo, upperBound(nums, target) - 1}
      ...
      },
      {
        name: "LIS tails",
        filename: "lis.go",
        note: "tails[k] is the smallest possible tail of an increasing subsequence of length k+1.",
  ...
  },
  {
    ...
    mentalModel: {
      lines: [
        "Same search, predicate a[i] > target.",
        "upperBound(x) - lowerBound(x) is the number of copies of x.",
        "upperBound(x) alone is the number of values <= x.",
      ],
      diagram: `  [ 1  2  2  2  5 ]   target = 2
  ...
  copies of 2 = 4 − 1 = 3`,
      key: "Lower bound counts strictly-less; upper bound counts less-or-equal.",
    },
    templates: [
      {
        name: "Upper bound",
    ...
    return lo
      ...
      },
      {
        ...
        note: "Three counts fall straight out of the two bounds.",
...
countInRange  := upperBound(a, hi) - lowerBound(a, lo) // [lo, hi] inclusive`,
  ...
  },
  {
    ...
    recognition: [
      ...
      "you can write a cheap check(x) that says whether x is good enough",
      "check is monotone: if x works, every larger (or smaller) x works too",
      "the phrasing is 'minimise the maximum' or 'maximise the minimum'",
    ],
    ...
    mentalModel: {
      lines: [
        ...
        "Write feasible(x) as a simple greedy or counting sweep.",
        "The sequence F F F T T T has exactly one boundary — find it.",
      ],
  ...
  cost: O(log range × cost of feasible)`,
      key: "The binary search is trivial. Designing feasible(x) is the whole problem.",
    },
    templates: [
      {
        name: "Minimum feasible",
        filename: "search_on_answer.go",
        note: "The skeleton. Only feasible() changes between problems.",
    ...
    return minFeasible(lo, hi, feasible)
      ...
      },
      {
        ...
        note: "Lower bound is max(nums) — no part can be smaller than its largest element.",
    ...
    return minFeasible(lo, hi, feasible)
      ...
      },
      {
        ...
        note: "The mirror image: feasible is true-then-false, so keep the last true.",
    ...
    why: "Binary search needs only a monotone predicate, not a sorted array. If feasible(x) implies feasible(x+1), the answer space behaves exactly like a sorted array of booleans — and finding the flip point is a standard lower bound.",
    variations: [
    ...
    mistakes: [
      ...
      { title: "A non-monotone predicate", detail: "If feasible flips more than once, binary search finds a boundary — just not the one you wanted." },
  ...
  },
  {
    ...
    mentalModel: {
      lines: [
        "Split at mid. At least one side is a clean sorted run.",
        "Identify the sorted side, then ask whether the target lies inside it.",
        "If it does, search there; otherwise search the other side.",
      ],
  ...
  a[lo] <= a[mid] → left half sorted`,
    ...
    templates: [
      {
        ...
        note: "Inclusive bounds read more naturally here. Note <= in the left-sorted test.",
    ...
    return -1
      ...
      },
      {
        ...
        note: "Compare mid with hi. Comparing with lo does not distinguish the rotated case.",
    ...
    return nums[lo]
      ...
      },
      {
        ...
        note: "When nums[mid] == nums[hi] you learn nothing — shrink hi by one. Worst case O(n).",
  ...
  },
  {
    ...
    mentalModel: {
      lines: [
        ...
        "Going left strictly decreases; going down strictly increases.",
  ...
  v < target → move down (drop a row)`,
      key: "A corner where the two directions disagree is a decision procedure.",
    },
    templates: [
      {
        name: "Staircase",
        filename: "staircase.go",
        note: "O(m+n): each step removes an entire row or column.",
        code: `func searchMatrix(m [][]int, target int) bool {
    if len(m) == 0 {
        return false
    }
    ...
    return false
      ...
      },
      {
        ...
        note: "Double the bound until you overshoot, then binary search. See exponential search.",
    ...
    complexity: [
      ...
      { label: "Flattened 2D search", value: "O(log mn)", note: "only valid when rows continue into each other" },
  ...
  },
  {
    ...
    recognition: [
      "the function rises then falls (or falls then rises) exactly once",
      "you are optimising over a continuous or integer parameter, not searching for equality",
      "the derivative is monotone — even if you cannot write it down",
    ],
    typicalQuestion: "Minimise a convex cost function over a range of integers.",
    mentalModel: {
      lines: [
        "Pick two interior points m1 < m2.",
        "If f(m1) < f(m2) the maximum cannot be left of m1 — discard [lo, m1].",
        "Each round removes about a third of the range.",
      ],
      diagram: `        ╱‾‾╲
      ╱      ╲
    ╱          ╲
  ...
  f(m1) > f(m2) → hi = m2`,
      key: "Binary search needs a monotone predicate; ternary search needs a unimodal function.",
    },
    templates: [
      {
        name: "Integer ternary",
        filename: "ternary_int.go",
        note: "For integers, binary search on the slope f(m+1) - f(m) is simpler and exact.",
...
func ternaryMaxInt(lo, hi int, f func(int) int) int {
    ...
    return best
      ...
      },
      {
        ...
        note: "The cleaner integer form: the slope is monotone, so binary search it.",
...
func peakIndex(f func(int) int, lo, hi int) int {
    ...
    return lo
      ...
      },
      {
        ...
        note: "Fixed iteration count beats an epsilon comparison — 200 rounds is plenty.",
    ...
    complexity: [
      { label: "Time", value: "O(log n)", note: "base 3/2; two evaluations per round" },
      { label: "Slope binary search", value: "O(log n)", note: "one evaluation per round — prefer it" },
      { label: "Space", value: "O(1)" },
    ],
    variations: [
  ...
  },
  {
    ...
    mentalModel: {
      lines: [
        "Optimising is hard. Deciding is easy.",
        "Replace 'what is the best x?' with 'is x good enough?'",
        "That decision is monotone, so binary search finds the threshold.",
      ],
  ...
  decision:      can we split with every part <= X ?
         ...
         ▼  monotone in X
  binary search X`,
      key: "The reduction is the insight; the binary search is bookkeeping.",
    },
    templates: [
      {
        name: "Reduction",
        filename: "parametric.go",
        note: "Write decide() first. If it is monotone you are done.",
...
func parametric(lo, hi int, decide func(int) bool) int {
    ...
    return lo
      ...
      },
      {
        ...
        note: "Minimise the largest edge on a path: decide with a plain reachability search.",
    ...
    complexity: [
      { label: "Time", value: "O(D · log R)", note: "D = cost of one decision, R = parameter range" },
      { label: "Space", value: "O(D)", note: "whatever the decision procedure needs" },
    ],
    why: "Optimisation and decision are usually equivalent in difficulty up to a log factor. If decide(X) is monotone in X, log R decisions locate the exact optimum — which is why 'minimise the maximum' is nearly always a binary search in disguise.",
    variations: [
    ...
    mistakes: [
      { title: "A decision procedure that is not monotone", detail: "Verify the implication explicitly: decide(X) ⟹ decide(X+1)." },
  ...
  },
  {
    ...
    mentalModel: {
      lines: [
        "Probe 1, 2, 4, 8, 16 … until the value passes the target.",
        "The answer is now inside [bound/2, bound] — a range of size bound/2.",
  ...
  O(log p) probes + O(log p) search`,
      key: "Doubling costs the same asymptotically but touches far less data when the answer is early.",
    },
    templates: [
      {
        name: "Exponential search",
        filename: "exponential.go",
        note: "Guard the probe so an out-of-range read never happens.",
    ...
    return -1
      ...
      },
      {
        ...
        filename: "gallop.go",
        note: "Used inside Timsort-style merges: skip long runs from one side quickly.",
...
func gallop(a []int, start, target int) int {
    ...
    complexity: [
      ...
      { label: "Total", value: "O(log p)", note: "independent of n — that is the point" },
      { label: "Space", value: "O(1)" },
    ],
    variations: [
  ...
  },
  {
    ...
    recognition: [
      ...
      "you need any local maximum, and neighbours outside the array count as -infinity",
      "binary search applies because the slope is monotone even when the values are not",
    ],
    typicalQuestion: "Find a peak element in O(log n).",
    mentalModel: {
      lines: [
        "Compare a[mid] with a[mid+1] — that is the local slope.",
        "Climbing means a peak exists to the right; descending means one exists at or left of mid.",
        "The slope flips exactly once, which is the monotone predicate binary search needs.",
      ],
      diagram: `        ╱‾╲
      ╱     ╲
    ╱          ╲
  ...
  a[mid] > a[mid+1] → go left (mid may be the peak)`,
    ...
    templates: [
      {
        name: "Peak index",
        filename: "peak.go",
        note: "Works even for a plain unsorted array with distinct neighbours.",
    ...
    return lo
      ...
      },
      {
        ...
        note: "Peak first, then one ascending and one descending binary search.",
